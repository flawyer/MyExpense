...
import { ExpenseType, columns } from "./column";
...
async function getData(): Promise<ExpenseType[]> {
  try {
    const response = await axios.get(process.env.NEXT_PUBLIC_API+'api/ExpenseType');
    return response.data.map((item: ExpenseType, index: number) => ({
      ...item,
      sn: index + 1,
    }));
  } catch (error) {
...
export default function DemoPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    try {
      await axios.post(process.env.NEXT_PUBLIC_API +'api/ExpenseType', { name: formData });
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      if(x=== true){
      await axios.delete(process.env.NEXT_PUBLIC_API+`api/ExpenseType/${id}`);
  ...
  const actionColumn: ColumnDef<ExpenseType> = {
    accessorKey: "action",
    header: "Action",
    cell: ({ row }) => (
      <button
