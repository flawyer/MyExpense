...
async function getData(): Promise<ExpenseType[]> {
  try {
    const response = await axios.get('https://expenseincomeapi.onrender.com/api/ExpenseType');
...
export default function DemoPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    try {
      await axios.post('https://expenseincomeapi.onrender.com/api/ExpenseType', { name: formData });
      ...
      <div className="flex flex-col md:flex-row h-screen">
        ...
        <div className="w-full md:w-3/4 bg-gray-200">
          <form className="w-23 p-3" onSubmit={handleSubmit}>
            <label htmlFor="expenseForm">Expense Title</label>
            <input
              type="text"
              value={formData}
