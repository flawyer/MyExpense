...
async function getData(): Promise<IncomeSource[]> {
  try {
    const response = await axios.get(process.env.NEXT_PUBLIC_API+'api/IncomeSource');
...
export default function DemoPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    try {
      await axios.post(process.env.NEXT_PUBLIC_API +'api/IncomeSource', { name: formData });
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      if(x=== true){
      await axios.delete(process.env.NEXT_PUBLIC_API+`api/IncomeSource/${id}`);
  ...
  const actionColumn: ColumnDef<IncomeSource> = {
    accessorKey: "action",
    header: "Action",
    cell: ({ row }) => (
      <button
